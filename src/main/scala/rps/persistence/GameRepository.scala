...
trait GameRepository {
  def save(play: Play): Future[UUID]

  def readLastMatch(): Future[Option[Play]]
}

object GameRepository {
  def create(db: DatabaseDef)(implicit ec: ExecutionContext) =
    new GameRepository {
      val moveSerializer = CaseEnumSerialization[Move]
      val resultSerializer = CaseEnumSerialization[GameResult]
      ...
      def readLastMatch(): Future[Option[Play]] =
        db.run(Match.sortBy(_.occurredAt.desc).take(1).result.headOption)
          .map(_.flatMap(convertPlayRow)) // TODO approfondire cosa fa questa riga
